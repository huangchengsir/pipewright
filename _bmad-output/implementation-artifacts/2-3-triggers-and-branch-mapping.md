...
### 环境
- Go 在 `~/sdk/go/bin`(`export PATH`);拉依赖加 `dangerouslyDisableSandbox`。npm=npmmirror。测试 vault/密钥路径需 `PIPEWRIGHT_MASTER_KEY`。
...
- 后端:`gofmt -l`(空)、`go vet . ./cmd/... ./internal/...`(干净)、`go test ./...`(全绿)、`CGO_ENABLED=0 go build ./cmd/pipewright`(ok)、`go mod tidy`(无变更)、`make mem-check` = 20 MB(≤100 MB,未回退)。
...
- cmd/pipewright/main.go (修改:装配 trigger 服务)
