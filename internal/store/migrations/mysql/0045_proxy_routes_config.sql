-- 0045 proxy_routes.config(MySQL):域名运营增强(R2)的「高级配置」JSON 列。
...
-- MySQL 的 TEXT 用表达式默认值 DEFAULT (''),与既有 *_json 列(0008/0010 等)一致。
ALTER TABLE proxy_routes ADD COLUMN config LONGTEXT NOT NULL DEFAULT ('');
